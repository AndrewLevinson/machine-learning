...
def submission_checker(fp):
    ...
    def get_zip_name():
        ...
        files = files = [f for f in os.listdir(
            fp) if os.path.isfile(os.path.join(fp, f))]
        n_zips = 0
        for f in files:
            if (f.split('.')[1] == 'zip'):
                zf = f
            ...
            if n_zips > 1:
                raise ValueError(
                    'There is more than one zip file in this directory. It should only include a single zip file.')
        return(zf)
    ...
    def un_zip_file():
        ...
        if zip_file == '':
            raise ValueError(
                'There is no zip file in the specified directory.')
        zffp = fp + '/' + zip_file
    ...
    def get_ipynb_name():
        ...
        zf = ''  # name of file
        files = files = [f for f in os.listdir(
            fpt) if os.path.isfile(os.path.join(fpt, f))]
        n_zips = 0
        for f in files:
            if (f.split('.')[1] == 'ipynb'):
                zf = f
            ...
            if n_zips > 1:
                raise ValueError(
                    'There is more than one ipynb file in this directory. It should only include a single ipynb file.')
        return(zf)
    ...
    def check_files():
        ...
        for f in files:
            if f.split('.')[1] not in ft_allowed:
                raise ValueError(
                    'Invalid file type; only .ipynb, .pkl, and .py file extensions permitted')
            if f.split('.')[1] == 'py':
                if f.split('.')[0] != 'my_measures':
                    print(
                        'WARNING: Invalid .py file; `my_measures.py` is the only .py file permitted')
...
class BinaryClassificationPerformance():
    ...
    def handle_code(c):
        ...
        if c.find('my_measures') >= 0:
            ...
            final_script += c + '\n'
            final_script += 'pm = ' + \
                str(compute_measures_object) + ".performance_measures \n"
